...
@st.cache_resource
def load_models():
    # Cloud fallback: Force CPU if GPU isn't available
    ...
    # Use map_location=device to prevent errors on CPU-only servers
...
class ComparisonProcessor(VideoProcessorBase):
    ...
    def recv(self, frame):
        ...
        # On CPU cloud servers, we must use a lower resolution for speed
        ...
        if results.multi_face_landmarks:
            ...
            # Map landmarks back to original size for drawing
            self.mp_drawing.draw_landmarks(img, landmarks, mp.solutions.face_mesh.FACEMESH_TESSELATION, None, self.drawing_spec)
            
            # Predict every 6 frames to handle CPU bottlenecks
            if self.frame_count % 6 == 0:
                coords = np.array([[lm.x, lm.y, lm.z] for lm in landmarks.landmark])
                x1, y1 = int(min(coords[:,0])*w_orig), int(min(coords[:,1])*h_orig)
                x2, y2 = int(max(coords[:,0])*w_orig), int(max(coords[:,1])*h_orig)
                
                crop = img_rgb[max(0, y1):y2, max(0, x1):x2]
                if crop.size > 0:
                    pil_crop = Image.fromarray(crop)
                    tx = T.Compose([T.Resize((224, 224)), T.Grayscale(3), T.ToTensor(), T.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
                    with torch.no_grad():
                    ...
                    p_stack = 0.55 * p_rn + 0.45 * p_cb
                    self.smooth_cnn = (self.smooth_cnn * 0.6) + (p_rn * 0.4)
                    self.smooth_stack = (self.smooth_stack * 0.6) + (p_stack * 0.4)
                    
                    self.cnn_res = f"CNN: {CLASSES[np.argmax(self.smooth_cnn)]} ({int(np.max(self.smooth_cnn)*100)}%)"
                    self.stack_res = f"STACKING: {CLASSES[np.argmax(self.smooth_stack)]} ({int(np.max(self.smooth_stack)*100)}%)"

            cv2.putText(img, self.cnn_res, (20, 40), cv2.FONT_HERSHEY_DUPLEX, 0.7, (255, 100, 0), 2)
            cv2.putText(img, self.stack_res, (20, 80), cv2.FONT_HERSHEY_DUPLEX, 0.7, (255, 0, 255), 2)
